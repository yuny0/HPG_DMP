...
class HazeOnlineDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img_gt is None:
            # 文件可能不存在，或者不是一个有效的图像文件
            print(f"Error: Unable to load image '{gt_path}'. Please check the file path and file format.")
        else:
            # 将图像数据转换为浮点型并归一化
            img_gt = img_gt.astype(np.float32) / 255.0
        depth_path = os.path.join(self.depth_folder, os.path.splitext(os.path.basename(gt_path))[0] + '.npy')
        ...
        return {
            ...
            'lq_path': gt_path,
            'gt_path': gt_path,
            'depth_transform_gt': img_gt,
            'depth_transform_lq': img_lq
        }
