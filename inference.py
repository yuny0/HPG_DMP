...
from basicsr.archs.femasr_arch import HPGDMPNet
...
def main():
    ...
    parser.add_argument('-i', '--input', type=str, default='/data_8T1/root/dataset/Teton_0-0', help='Input image or folder')
    ...
    parser.add_argument('-w_HQ', '--weight_HQ', type=str,
                        default='/data/root/ICCV23/FeMaSR_HRP_model_g.pth',
    ...
    for idx, path in enumerate(paths):
        ...
        for prompt in range(prompt_number):
            ...
            # set up the model
            HQ_model = HPGDMPNet(codebook_params=[[32, 1024, 512]], LQ_stage=False, scale_factor=args.out_scale).to(
                device)
            HQ_model.load_state_dict(torch.load(weight_path_HQ)['params'], strict=False)
            HQ_model.eval()
            Dehaze_model = HPGDMPNet(codebook_params=[[32, 1024, 512]], LQ_stage=True, scale_factor=args.out_scale).to(
                device)
